...
export const api = {
  ...
  async verifyTestRazorpayPayment(test_id: string, razorpay_payment_id: string, razorpay_order_id?: string, razorpay_signature?: string): Promise<{
    success: boolean;
    message: string;
    user: UserProfile;
  }> {
    const res = await fetch('/api/payments/razorpay/verify-test-payment', {
      method: 'POST',
      headers: headers(),
      body: JSON.stringify({ test_id, razorpay_payment_id, razorpay_order_id, razorpay_signature })
    });
    return res.json();
  },
  ...
  async verifyLectureRazorpayPayment(lecture_id: string, razorpay_payment_id: string, razorpay_order_id?: string, razorpay_signature?: string): Promise<{
    success: boolean;
    message: string;
    ...
    const res = await fetch('/api/payments/razorpay/verify-lecture-payment', {
      method: 'POST',
      headers: headers(),
      body: JSON.stringify({ lecture_id, razorpay_payment_id, razorpay_order_id, razorpay_signature })
    });
    return res.json();
  },
  ...
  async deleteUsers(ids: string[], deletePassword: string): Promise<{ success:boolean; deleted:number; skipped:number }> {
    const res = await fetch('/api/admin/users', { method:'DELETE', headers:headers(), body:JSON.stringify({ ids, deletePassword }) });
    const data = await res.json(); if(!res.ok) throw new Error(data.error || 'Failed to delete students'); return data;
  },

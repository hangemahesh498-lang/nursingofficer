import { Subject, Chapter, Topic, CaseStudy, Question, MockTest } from '../types';

const RAW_INITIAL_SUBJECTS: Subject[] = [
  {
    "id": "subj-fon",
...
export const INITIAL_QUESTIONS: Question[] = [
  ...
  },
  {
    ...
    "correct_option": "C",
    "explanation_en": "Parkland Formula = 4 mL × Weight in kg × % TBSA burned. Here: 4 mL × 70 kg × 40 = 11,200 mL of Ringer Lactate in 24 hours. Half of this total (5,600 mL) must be infused in the first 8 hours post-burn injury.",
    "explanation_mr": "पार्कंड सूत्र: ४ मिली × वजन (किलो) × भाजलेली टक्केवारी (%). गणना: ४ × ७० × ४० = ११,२०० मिली. यातील निम्मे (५,६०० मिली) पहिल्या ८ तासांत द्यावे लागते.",
    "difficulty": "medium",
    "question_type": "single_best",
    "exam_tags": [
      "NORCET",
...
export const INITIAL_SUBJECTS: Subject[] = RAW_INITIAL_SUBJECTS.map(s => {
  const publishedQs = INITIAL_QUESTIONS.filter(q =>
    (!q.status || q.status === 'published') &&
    (q.subject_id === s.id || INITIAL_CHAPTERS.some(c => c.id === q.chapter_id && c.subject_id === s.id))
  );
  return {
    ...s,

  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="flex flex-wrap items-center justify-center gap-2 p-1.5 bg-slate-100 rounded-2xl max-w-2xl mx-auto border border-slate-200">
        ...
        <button type="button" onClick={() => setSelectedPlanTypeFilter('YOUTUBE')} className={`px-4 py-2 rounded-xl text-xs font-bold transition cursor-pointer ${selectedPlanTypeFilter === 'YOUTUBE' ? 'bg-red-600 text-white' : 'text-slate-600 hover:text-slate-900'}`}>▶️ YouTube Videos</button>
        <button type="button" onClick={() => setSelectedPlanTypeFilter('TEST_SERIES')} className={`px-4 py-2 rounded-xl text-xs font-bold transition cursor-pointer ${selectedPlanTypeFilter === 'TEST_SERIES' ? 'bg-indigo-700 text-white' : 'text-slate-600 hover:text-slate-900'}`}>📝 Test Series</button>
